  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4 gap-4">
        <div>
          <h2 className="text-xl font-semibold">Kelola Komoditas</h2>
          <div className="text-sm text-gray-600">Daftar komoditas dan pengaturan cepat</div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={openAdd} className="px-3 py-2 bg-green-600 text-white rounded-md hover:bg-green-700">Tambah Komoditas</button>
        </div>
      </div>
        ...
        <table className="w-full min-w-[600px]">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-sm font-medium text-gray-700 border-b">Nama Komoditas</th>
          ...
          <tbody>
            {filtered.map((r) => (
              <tr key={r.id} className="even:bg-gray-50">
                ...
                <td className="px-4 py-3 align-top text-sm text-gray-900">{r.nama_komoditas}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 grid place-items-center">
          <div className="bg-white p-6 rounded shadow w-96">
            <h3 className="font-medium mb-3">{editing?.id ? 'Edit Komoditas' : 'Tambah Komoditas'}</h3>
            <input className="w-full p-2 border rounded mb-2" value={editing?.nama_komoditas ?? ''} onChange={(e) => setEditing(prev => prev ? {...prev, nama_komoditas: e.target.value} : { nama_komoditas: e.target.value })} />
