  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT id, nip, nama_lengkap, username, password, role, is_active,
              phone, alamat, foto, updated_at
       FROM users
       WHERE username=? OR nip=? LIMIT 1`,
      [identity, identity]
    );
    const u = rows?.[0];
...
router.get('/me', authMobile, async (req, res) => {
  const [[u]] = await pool.query(
    `SELECT id, nip, nama_lengkap, username, role, is_active,
            phone, alamat, foto, updated_at
     FROM users
     WHERE id=? LIMIT 1`,
    [req.mobile.uid]
...
router.post('/change-password', authMobile, async (req, res) => {
  try {
    ...
    const [[u]] = await pool.query('SELECT id, password FROM users WHERE id=? LIMIT 1', [req.mobile.uid]);
    ...
    await pool.query('UPDATE users SET password=?, updated_at=NOW() WHERE id=?', [hash, req.mobile.uid]);
    res.json({ ok: true });
  } catch (e) {
