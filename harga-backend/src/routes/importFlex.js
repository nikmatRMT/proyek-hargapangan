...
router.post("/api/import-flex", async (req, res) => {
  ...
  const komMap = makeKomoditasMap(komList);
    ...
    if (truncate && year && month) {
      ...
      await laporan_harga.deleteMany({ market_id: marketId, tanggal_lapor: { $gte: start, $lt: end } });
    }
    ...
    for (const r of prepared) {
      ...
      await laporan_harga.updateOne(
        { market_id: marketId, komoditas_id: r.komoditas_id, tanggal_lapor: r.tanggal },
        { $set: { harga: r.harga, keterangan: r.keterangan || null, updated_at: new Date() }, $setOnInsert: { id: newId, created_at: new Date() } },
        { upsert: true }
      );
      imported++;
    }
